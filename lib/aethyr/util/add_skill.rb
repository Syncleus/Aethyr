require 'yaml'
require 'aethyr/combatskill'

if File.exist? "lib/aethyr/combat_skill.yaml"
  skills = YAML.load("aethyr/combat_skill.yaml")
else
  skills = {}
end
...
loop do
  ...
  File.open("lib/aethyr/combat_skill.yaml", "w") { |f| YAML.dump(skills, f) }
