...
module Combat
  ...
  #Keeps track of 'in-flight' combat commands.
  class CombatEventTracker
    ...
    #Display the commands currently in the list nicely.
    def inspect
      @events.map do |e|
        e.to_s
      end.join(", ")
  ...
  class << self
    ...
    #Find commands matching the given args.
    ...
    #
    #If there are already commands which target this player,
    #the event will be attached to those commands so it ends
    #at the same time.
    #
