        ...
        class NoCommand < Aethyr::Extend::EmoteAction
          ...
          def action
            ...
            make_emote self, player, room do
              ...
              target do
                to_player  "You shake your head, disagreeing with #{self.target.name}."
                to_target "#{player.name} shakes #{player.pronoun(:possessive)} head in your direction, disagreeing."
                to_other "#{player.name} shakes #{player.pronoun(:possessive)} head in disagreement with #{self.target.name}."
                to_deaf_other self[:to_other]
              end
            end
