    ...
    module Integration
      ...
      class ServerHarness
        ...
        def configure_runtime_port!
          ...
          # ----------------------------------------------------------------
          # Ensure that *all* log output generated by the in-process Aethyr
          # server is surfaced to the calling test runner.  Historically the
          # bundled test fixtures ship with `log_level: 0` which effectively
          # mutes **every** log entry because the Logger implementation only
          # emits messages when the *configured* level is *strictly greater*
          # than the message severity.  This makes diagnosing failures during
          # CI or local development exceedingly painful because no diagnostic
          # information is visible.

          # To guarantee visibility we aggressively elevate the in-memory
          # configuration value to a sentinel value that exceeds *all*
          # current severity constants (Important→0, Normal→1, Medium→2,
          # Ultimate→3).  Using `Integer::MAX` protects against future
          # additions without requiring another patch.
          #
          # NOTE: We deliberately avoid persisting this change back to disk –
          # it lives only for the duration of the harness lifecycle ensuring
          # no side-effects leak into the developer's workspace.
          # ----------------------------------------------------------------
          ServerConfig.load[:log_level] = (defined?(Integer::MAX) ? Integer::MAX : 1 << 30)
        end
