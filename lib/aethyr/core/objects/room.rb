    ...
    module Objects
      ...
      class Room < Container
        ...
        def add(object)
          ...
          # if the item is a living thing, tell everyone in the room that
          # the thing has arrived.
          if object.is_a? Aethyr::Core::Objects::Player or object.is_a? Mobile
            return object
          end
          return object
        end
        ...
        def players(only_visible = true, exclude = nil)
          players = Array.new
          # add each person or mob to the list, unless they should be excluded
          self.inventory.each do |item|
            players << item if item.is_a?(Aethyr::Core::Objects::Player) and item != exclude and (!only_visible or item.visible)
          end
          players
        end

        # Return mobiles in the room. Don't count Players.
        def mobiles(only_visible = true, exclude = nil)
          mobs = Array.new
          self.inventory.each do |item|
            mobs << item if (!only_visible or item.visible) and item.can? :alive and item.alive and !item.is_a?(Aethyr::Core::Objects::Player)
          end
          mobs
        end
        ...
        def look(player)
          ...
          @inventory.each do |item|
            ...
            if item.is_a?(Aethyr::Core::Objects::Player) and item != player and item.visible
              if item.pose
